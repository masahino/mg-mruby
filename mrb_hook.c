...
mrb_hook_callback(char *hook_name, struct buffer *bp)
{
    ...
    hook_list = mrb_mod_cv_get(mrb, mrb_class_get(mrb, "MG"), mrb_intern(mrb, "@@hook_list"));
...
mrb_add_hook(mrb_state *mrb, mrb_value self)
{
    ...
    hook_list = mrb_cv_get(mrb, self, mrb_intern(mrb, "@@hook_list"));
...
void mrb_hook_init(mrb_state *mrb)
{
     ...
     mrb_mod_cv_set(mrb, mg, mrb_intern(mrb, "@@hook_list"), hook_list);
